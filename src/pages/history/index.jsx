...
const History = () => {
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      const hasPermission =
        user.roleId === 1 ||
        (user.permissions && user.permissions.includes("history_view"));
  ...
  const fetchLogs = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        console.log(
          `History: Obteniendo historial... (${silent ? "silencioso" : "completo"})`,
        );
        ...
        if (Array.isArray(response.data)) {
          ...
          console.warn(
            "History: Respuesta no es un array, intentando convertir:",
            response.data,
          );
          dataArray = [];
        } else {
          console.error(
            "History: Formato de respuesta inválido:",
            response.data,
          );
          dataArray = [];
        }
  ...
  const getActionColor = useCallback(
    (accion) => {
      ...
      if (actionLower.includes("elimin") || actionLower.includes("borrar"))
        return actionColors["eliminado"];
      if (
        actionLower.includes("actualiz") ||
        actionLower.includes("modific") ||
        actionLower.includes("edit")
      )
        return actionColors["actualizado"];
      if (
        actionLower.includes("crea") ||
        actionLower.includes("registra") ||
        actionLower.includes("nuevo")
      )
        return actionColors["creado"];
      ...
      >
        <Table stickyHeader>
          <TableHead sx={{ backgroundColor: safeColors.blueAccent[700] }}>
            <TableRow>
              <TableCell>
                <Typography fontWeight="bold">Fecha y Hora</Typography>
              </TableCell>
              <TableCell align="center">
                <Typography fontWeight="bold">Acción</Typography>
              </TableCell>
              <TableCell>
                <Typography fontWeight="bold">Realizado por</Typography>
              </TableCell>
              <TableCell>
